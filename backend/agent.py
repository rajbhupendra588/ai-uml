...
_TIER_ORDER = ("balancer", "client", "auth", "server", "function", "queue", "database")
...
def generator_node(state: AgentState):
    ...
    # HLD diagram
    if diagram_type == "hld":
        mermaid_code = _hld_to_mermaid(plan)
        return {"json_output": {"mermaid": mermaid_code, "nodes": [], "edges": []}}

    # Architecture diagram
    if diagram_type == "architecture":
        components = plan.get("components", [])
        if not components:
            return {"json_output": {"mermaid": "", "nodes": [], "edges": []}}
        mermaid_code = _architecture_to_mermaid(components)
        return {"json_output": {"mermaid": mermaid_code, "nodes": [], "edges": []}}
